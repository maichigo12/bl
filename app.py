...
st.markdown("""
### 🎨 シュールで奇妙なキャラクターを自動生成！
このアプリは、AIを使ってイタリアン・ブレインロット風のシュールなキャラクターを生成します。
""")
...
def generate_character_data():
    ...
    """
    system_prompt = (
        "あなたは「イタリアン・ブレインロット」風のキャラクターを生成するクリエイターです。"
        "シュールで混沌とした、少し不気味だけど面白いキャラクターを考えてください。"
        "**ただし、キャラクターの要素はイタリア料理に限定せず、動物、日常品、他の文化の要素などもランダムに組み合わせてください。**"
